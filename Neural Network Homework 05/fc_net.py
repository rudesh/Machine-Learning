...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(layers-1):
            ...
            #for batchnorm
            if self.use_batchnorm:
                batchnorm_out, batchnorm_cache[str(layer+1)] = batchnorm_forward(affine_out, self.params['gamma'+str(layer+1)], self.params['beta'+str(layer+1)], self.bn_params[layer])
                tuple_out, relu_cache[str(layer+1)] = relu_forward(batchnorm_out)
